...
| `schema_model.svg` | the **published** schema.org graph documented in full: every class, every property edge, and the counts behind them. Generated by `document_schema_model.py` from the data file, so a class or property that disappears is reported rather than silently dropped from the picture. |
| `conceptual_model.svg` | the class diagram, standalone with light/dark support. Drawn from the **published** data file when one is present, since that is the model consumers read; box labels and edge property names both come from queries, so neither can drift. Falls back to the internal graph model if no published file is found. |
